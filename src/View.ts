...
class View {
  ...
  // движение бегунков
  moveRunnerAtValue(options: Options, runnerFirst?: HTMLElement, runnerSecond?: HTMLElement): void {
    const { to, from, orientation } = this.options;
    const convertTo = this.convertingValueToPx(to, this.options);
    const convertFrom = this.convertingValueToPx(from, this.options);
    const posTo = this.convertingPxToPercentages(convertTo, this.options);
    const posFrom = this.convertingPxToPercentages(convertFrom, this.options);

    if (orientation === 'horizontal') {
      runnerFirst!.style.left = `${posFrom}%`;
      runnerSecond!.style.left = `${posTo}%`;
    } else {
      runnerFirst!.style.bottom = `${posFrom}%`;
      runnerSecond!.style.bottom = `${posTo}%`;
    }
  }
  ...
  convertingValueToPx(value: number, options: Options): number {
    return (value * 100) / this.getSize(options);
  }
  ...
  newPosition(value: number, target?: HTMLElement): void {
    ...
    //   const pxToPerc = this.convertingPxToPercentages(position, options);
